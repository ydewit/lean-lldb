...
test: build-image
	docker run \
		...
		-e PYTHONHASHSEED=1 \
		lean-lldb:$(DOCKER_IMAGE_TAG) \
		bash -c "ulimit -n 1024 && cd /root/.lldb/lean-lldb && poetry run pytest -n auto -vv tests/"
...
debug: build-image
	docker run -t -i \
		...
		-e PYTHONHASHSEED=1 \
		lean-lldb:$(DOCKER_IMAGE_TAG) \
		bash -c "ulimit -n 1024 && cd /root/.lldb/lean-lldb && poetry run pytest -s -n auto -vv --pdb -x --ff tests/"

# Start a shell in a container with lean-lldb installed
shell: build-image
	docker run -t -i \
		--rm \
		--security-opt seccomp:unconfined --cap-add=SYS_PTRACE \
